...
const SignUpForm = () => {
  ...
  // const [cfrmPassword,setCfrmPassword] = useState<string | null>(null)
  ...
  const onSubmit: SubmitHandler<FormData> = useCallback(async(values) => {
    ...
    try {
      const { data } = await createUser({
        variables: { user_name: values.name, user_email: values.email,user_phone:values.mobileNumber, user_password: values.password,user_role:values.role },
      });
    ...
    <div className='container'>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='fields'>
          ...
          <p className='footer'>Already have an account? <span className='highlight' onClick={()=>navigate('/login')}>Login</span> </p>
        </div>
      </form>
    </div>
  )
